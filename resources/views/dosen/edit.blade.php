...
<div class="bg-white rounded-lg shadow p-6 max-w-lg mx-auto">
    ...
    <form action="{{ route('dosen.update', $dosen->nidn) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" value="{{ $dosen->nidn }}" class="w-full border rounded px-3 py-2 bg-gray-100" disabled>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 font-bold mb-2">Nama</label>
            <input type="text" name="nama" value="{{ $dosen->nama }}" maxlength="50" class="w-full border rounded px-3 py-2" required>
